...
| Commit | Cambio |
|---|---|
| _(actual)_ | **Chequeo de stop vs ATR cerrado en los radares:** el Radar de Opciones expone el ATR(14) diario que ya calculaba para el Keltner, y Derivados lo calcula con una descarga ligera — el validador ahora detecta stops que el ruido barrería (ej. stop 0.1x ATR con R/B 10 seductor) en los 3 módulos. **Extensión del pipeline verificado a los radares:** los reportes IA de Flujo de Opciones y Radar de Opciones ahora emiten el mismo JSON `{sesgo, direccion, entrada, stop, tp1}` y pasan por `validador_plan` — tarjeta Operación/Entrada/Stop/TP + avisos en ambos módulos. **Escáner Global:** columnas P/S, ROE y Deuda/EBITDA en la tabla de Oportunidades de Valor (tabla del auditor completa). |
| `95dee9d` | Acceso defensivo a claves del validador en la UI (desfase de recarga de módulos en Streamlit Cloud). |
